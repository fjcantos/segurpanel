...
| `APP_URL` / `BASE_URL` | Opcional en Render, recomendada fuera de Render | URL pública de la app (p.ej. `https://tu-app.onrender.com`), usada para construir enlaces absolutos en emails automáticos (aviso de nueva solicitud de acceso → enlace a `/admin`, recuperación de contraseña → enlace a `/reset-password`). En Render no hace falta definirla: se detecta sola a partir de `RENDER_EXTERNAL_URL` (que Render define automáticamente). Fuera de Render, sin ninguna de las dos se usa `http://localhost:PORT`, que no funciona como enlace fuera del propio servidor. |
...
5. **Variables de entorno** (Render → tu servicio → *Environment*):
   ...
   - `APP_URL` (o `BASE_URL`) — normalmente NO hace falta en Render: se detecta sola a partir de `RENDER_EXTERNAL_URL` (variable que Render define automáticamente con la URL pública real del servicio). Solo defínela a mano si quieres forzar un dominio propio distinto del que asigna Render.
